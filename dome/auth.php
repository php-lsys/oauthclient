...
$redirect_uri=OauthClient::create_redirect_uri("ref",$redirect_uri);//ref 为页面传递过来登录完成时返回地址$_GET变量的KEY
try{
	$client=$oauth->get_driver($type)->get_client($redirect_uri);//获取客户端对象
	$user=$client->get_user();
	$utoken=md5(json_encode($user));//得到用户标识,可以用用户ID
	var_dump($client->expires());//客户端剩余有效时间
}catch (\LSYS\OauthClient\Exception $e){
	print_r($e->getMessage());
	var_dump($e->need_login());
	if ($e->need_login()){
		//to login page
	}
	exit;
